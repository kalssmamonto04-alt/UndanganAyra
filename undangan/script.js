...
function populateContent(data) {
  ...
  setText('closingName', data.namaKeluarga || acara.namaAdik);
  ...
  // Amplop digital (tanda kasih / transfer)
  populateAmplop(data.amplop || {});
...
function populateAmplop(amplop) {
  const section = document.getElementById('amplopSection');
  if (!section) return;

  if (!amplop || amplop.aktif === false || !amplop.nomorRekening) {
    section.style.display = 'none';
    return;
  }

  setText('amplopJudul', amplop.judul || 'Kirim Doa & Tanda Kasih');
  setText('amplopKeterangan', amplop.keterangan);
  setText('amplopBank', amplop.bank);
  setText('amplopNomor', amplop.nomorRekening);
  setText('amplopNama', amplop.namaPemilik ? `a.n. ${amplop.namaPemilik}` : '');

  const copyBtn = document.getElementById('amplopCopyBtn');
  if (copyBtn) {
    copyBtn.addEventListener('click', () => {
      const nomor = amplop.nomorRekening || '';
      const doCopyFeedback = () => {
        ...
        setTimeout(() => {
          if (label) label.textContent = original || 'Salin Nomor Rekening';
          copyBtn.classList.remove('copied');
        }, 1800);
      };
      if (navigator.clipboard && navigator.clipboard.writeText) {
        navigator.clipboard.writeText(nomor).then(doCopyFeedback).catch(() => {
          prompt('Salin nomor rekening ini:', nomor);
        });
      } else {
        prompt('Salin nomor rekening ini:', nomor);
      }
